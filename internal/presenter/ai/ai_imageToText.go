...
type ImageToTextPresenter interface {
	ExtractText(ctx echo.Context) error
}

// imageToTextPresenter 實作 ImageToTextPresenter 介面
type imageToTextPresenter struct {
	Photo []byte `json:"Photo"`
}

// NewImageToTextPresenter 建立 ImageToTextPresenter 的實例
func NewImageToTextPresenter() ImageToTextPresenter {
	return &imageToTextPresenter{}
}
...
func (p *imageToTextPresenter) ExtractText(ctx echo.Context) error {
	// 1. 取得圖片
